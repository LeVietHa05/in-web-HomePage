...
export default function TrainghiemHs({ type }: { type: string }) {
    ...
    return (
        <div className="bg-[#EAFDFF]  w-full rounded-[32px] lg:rounded-[42px] xl:rounded-[60px]  relative z-2  overflow-x-hidden overflow-y-hidden -mt-12 lg:-mt-24 pt-8 lg:pt-16 lg:pb-16">
            ...
            <div className='flex w-fit pt-6' >

                {data.map((each, i) => {
                    return (
                        <div key={i} className=' flex flex-col lg:flex-row w-screen xl:w-[1280px] mx-auto transition-transform duration-500 ease-in-out px-3 lg:px-8 ' style={{ transform: `translateX(-${currentPage * 100}%)` }} >
